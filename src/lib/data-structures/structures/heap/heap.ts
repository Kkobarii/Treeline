...
export class Heap extends DataStructure {
	...
	insert(value: number, data: OperationData): HeapNode {
		...
		this.nodes.push(newNode);
		this.rebuildPointers();
		data.step(StepData.new(new AppendData(newNode.id, value, parent.id, startSnapshot, this.snapshot())));
	...
	extractRoot(data: OperationData): HeapNode | null {
		...
		if (this.nodes.length === 1) {
			...
			this.rebuildPointers();
			data.step(StepData.new(new DeleteData(removed.id, removed.value, startSnapshot, this.snapshot())));
			return removed;
		}
		...
		data.step(StepData.new(new MarkToDeleteData(root.id, root.value)));

		const swapStart = this.snapshot();
		this.swapNodes(0, lastIndex);
		this.rebuildPointers();
		data.step(StepData.new(new ReplaceRootWithLastData(root.id, lastNode.id, rootValue, lastValue, swapStart, this.snapshot())));

		const deleteStart = this.snapshot();
		const removed = this.nodes.pop()!;
		this.rebuildPointers();
		data.step(StepData.new(new DeleteData(removed.id, removed.value, deleteStart, this.snapshot())));
	...
	private bubbleUp(index: number, data: OperationData) {
		...
		while (currentIndex > 0) {
			...
			data.step(StepData.new(new CompareWithParentData(child.id, parent.id, needsSwap)));
			if (!needsSwap) break;
			...
			data.step(StepData.new(new SwapData(child.id, parent.id, childValue, parentValue, startSnapshot, endSnapshot)));
	...
	private bubbleDown(index: number, data: OperationData) {
		...
		while (true) {
			...
			if (largestIndex !== currentIndex) {
				data.step(
					StepData.new(
						new FindLargestChildData(this.nodes[currentIndex].id, this.nodes[largestIndex].id, this.nodes[largestIndex].value),
					),
				);
				...
				this.rebuildPointers();
				const endSnapshot = this.snapshot();
				data.step(StepData.new(new SwapData(currentNode.id, targetNode.id, currentValue, targetValue, startSnapshot, endSnapshot)));
